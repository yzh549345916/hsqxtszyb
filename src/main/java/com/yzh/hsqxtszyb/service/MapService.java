...
import com.yzh.hsqxtszyb.mapper.ZNWG.GuoJiaZNWGGd001003Mapper;
...
import com.yzh.hsqxtszyb.杨姐区局项目临时.杨姐区局数据处理;
...
import static com.yzh.hsqxtszyb.杨姐区局项目临时.杨姐区局数据处理.Rmaps格点详情数据处理;
...
public class MapService {
    ...
    public List<站点信息> 根据ID获取Rmaps站点(String ID, String StationType) {
        ...
        //杨姐区局项目 多模式风场网格预报本地化预报业务系统
        if (StationType.contains("多模式风场")) {
            sb.append("121,");
        }
        if (sb.length() > 0) {
            sb.deleteCharAt(sb.length() - 1);
       ...
       try{
           ...
           if(YBType.contains("RMAPS")){
               if(stationlevel==0&&stationlevelType==103){
                   ...
                   String myFilePath= StrUtil.format("/home/ftp/区台数值预报文件/szyb/json/rmaps数据/风流场/{}/RMAPS_wind_{}_{}_{}.json", datePath1,dlat,timePath1,String.format("%04d", YbSx) );;
    ...
    public OpenlayersQJMethodBaseModel 获取多模式风场网格预报站点预报数据(String YBType, String DataTypeID, String StationTye, String DQID, long times, int YbSx, int stationlevelType, double stationlevel) {
        try {
            int dataTypeIDInt = Integer.parseInt(DataTypeID);

            List<站点信息> ids;
           if (YBType.equals("RMAPS")) {
                ids = 根据ID获取Rmaps站点(DQID, StationTye);
                if (ids.size() > 0) {
    ...
    public 地图站点详情Model 获取多模式风场网格预报站点预报详情(String YBType, String DataTypeID, String StationID, long timespan,int stationlevelType, int stationlevel) {
        try {
            int dataTypeIDInt = Integer.parseInt(DataTypeID);
            DateTime sdate = DateUtil.date(timespan);
            if (YBType.equals("RMAPS")) {
                ...
                if (dataTypeIDInt == 4){
                    ...
                    地图站点详情表头Model[] headers=new 地图站点详情表头Model[5];
                    int myCount=0;
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","stationName","站名",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","timeStr","时间",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","ybValue1","风速(m/s)",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","ybValue2","风向",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","ybValue3","风力",true,false,true);
                    List<站点信息> myidList=stationDaoImpl.根据站点ID列表获取站点信息(new web站点检索Model(StationID));
                    String stationName="";
                    if(myidList.size()>0){
                        stationName=myidList.get(0).getName();
                ...
                {
                    地图站点详情表头Model[] headers=new 地图站点详情表头Model[3];
                    int myCount=0;
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","stationName","站名",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","timeStr","时间",true,false,true);
                    headers[myCount++]=new 地图站点详情表头Model(true,"center","ybValue1",myYsText+"("+myYsUnit+")",true,false,true);
    ...
    public String 获取多模式风场预报格点数据(String YBType, long times, int YbSx, int stationlevelType, double stationlevel){
        ///home/ftp/
        //F:\tmp\
        String HomeDirPath="F:\\tmp\\";
        try{
            DateTime myDate = DateUtil.date(times);
            if(YBType.contains("RMAPS")){
                if(stationlevel==0&&stationlevelType==103){
                    String myFilePath= 杨姐区局数据处理.获取Rmaps格点Json路径(HomeDirPath,myDate,YbSx);
                    if(FileUtil.exist(myFilePath)){
                        return FileUtil.readUtf8String(myFilePath);
                    }
                    else{
                        杨姐区局数据处理.Rmaps格点数据处理(HomeDirPath,myDate,YbSx);
                        if(FileUtil.exist(myFilePath)){
                            return FileUtil.readUtf8String(myFilePath);
                        }
                    }
            ...
            else if(YBType.contains("国家智能网格")){
                if(stationlevel==0&&stationlevelType==103){
                    String myFilePath= 杨姐区局数据处理.获取国家智能网格格点Json路径(HomeDirPath,myDate,YbSx);
                    if(FileUtil.exist(myFilePath)){
                        return FileUtil.readUtf8String(myFilePath);
                    }
                    else{
                        杨姐区局数据处理.国家智能网格格点数据处理(HomeDirPath,myDate,YbSx);
                        if(FileUtil.exist(myFilePath)){
                            return FileUtil.readUtf8String(myFilePath);
                        }
                    }
    ...
    public 地图站点详情Model 获取多模式风场网格预报格点预报详情(String YBType, String DataTypeID, String LonLat, long timespan,int stationlevelType, int stationlevel) {
        try {
            地图站点详情Model myData = new 地图站点详情Model();
            ...
            try{
                ...
                String[] LonLatSZ=LonLat.split(",");
                if(NumberUtil.isNumber(LonLatSZ[0])&&NumberUtil.isNumber(LonLatSZ[1])){
                    lon=NumberUtil.parseDouble(LonLatSZ[0]);
                    lat=NumberUtil.parseDouble(LonLatSZ[1]);
                }else{
                    myData.setTitle("数据获取异常");
                    return myData;
                }
                if(YBType.contains("RMAPS")){
                    if(stationlevel==0&&stationlevelType==103){
                        ...
                        if (dataTypeIDInt == 4){
                            ...
                            headers[myCount++]=new 地图站点详情表头Model(true,"center","ybValue3","风力",true,false,true);
                            myData.setHeaders(headers);
                            List<地图站点详情数据内容> xqdataList=new ArrayList<>();
                            for(int i=0;i<=96;i++){
                                try{
                                    地图站点详情数据内容 myItem=Rmaps格点详情数据处理(HomeDirPath,sdate,i,lon,lat);
                                    if(myItem!=null){
                                        xqdataList.add(myItem);
                                    }
                            ...
                            if(xqdataList.isEmpty()){
                                return myData;
                            }
                            myData.setDatas(xqdataList.toArray(new 地图站点详情数据内容[0]));
                            myData.setHeaders(headers);
                            return myData;
                        }
                    ...
                    if(stationlevel==0&&stationlevelType==103){
                        ...
                        if (dataTypeIDInt == 4){
                            ...
                            myData.setHeaders(headers);
                            List<地图站点详情数据内容> xqdataList=new ArrayList<>();
                            SqlSessionFactory sqlSessionFactoryZNWG = SqlSessionFactoryUtil.getZNWG203SessionFactory();
                            SqlSession sessionZNWG = sqlSessionFactoryZNWG.openSession();
                            GuoJiaZNWGGd001003Mapper gdMapper= sessionZNWG.getMapper(GuoJiaZNWGGd001003Mapper.class);
                            入库格点经纬度范围 Gdfw=gdMapper.selectFirstGDFWByMSName("GJZNWG");
                            //如果超出经纬度范围则返回空数据
                            if(!(lon<=Gdfw.getLonEnd()&&lon>=Gdfw.getLonStart()&&lat<=Gdfw.getLatEnd()&&lat>=Gdfw.getLatStart())){
                                return myData;
                            }
                            Long lonIndex=NumberUtil.round((lon-Gdfw.getLonStart())/ Gdfw.getLonStep(),0).toBigInteger().longValue();
                            Long latIndex=NumberUtil.round((lat-Gdfw.getLatStart())/ Gdfw.getLatStep(),0).toBigInteger().longValue();
                            List<GuoJiaZNWGGd001003> dataList=gdMapper.selectAllByLonindexAndLatindexAndQbdatetimeAndWiu10IsNotNullOrderByYbsx(lonIndex,latIndex,sdate.toLocalDateTime());
                            for (var dataItem:dataList
                                 ) {
                               try{
                                   地图站点详情数据内容 myItem=new 地图站点详情数据内容(DateUtil.format(DateUtil.offsetHour(Date.from(dataItem.getQbdatetime().toInstant(ZoneOffset.ofHours(8))),dataItem.getYbsx()), "MM月dd日HH时"),NumberUtil.round(风向风速转换.GetFS(dataItem.getWiv10(), dataItem.getWiu10()), 1).doubleValue(),风向风速转换.GetFX(dataItem.getWiv10(), dataItem.getWiu10()),风向风速转换.GetFL(dataItem.getWiv10(), dataItem.getWiu10()));
                                   xqdataList.add(myItem);
                               } catch (Exception e) {
                                   throw new RuntimeException(e);
                               }
